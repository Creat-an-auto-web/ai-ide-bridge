...
class RequirementCompositionVerificationPromptBuilder:
    ...
    def build_user_prompt(
        ...
        return (
            ...
            "要求：\n"
            "1. 输出必须是 json 对象。\n"
            "2. 这不是单条 story 文案检查，而是整组 story 的业务闭环检查。\n"
            "3. 必须检查主路径、权限路径、失败路径和跨 story 依赖是否完整。\n"
            "4. 如果当前 story 集合已经足以构造端到端验证场景，status 设为 pass。\n"
            "5. 如果存在可修复缺口，status 设为 revise，并给出 revision_guidance。\n"
            "6. 如果当前信息不足或 story 集合与原始需求严重冲突，status 设为 blocked。\n"
            "7. 必须给出 integration_test_scenarios，说明哪些 stories 组合起来构成完整验证场景。\n"
            "8. 不要重写所有 stories，只指出组合级问题和缺口。\n\n"
